...
const login = async (req, res) => {
  try {
    ...
    await query(
      'UPDATE users SET otp = ?, otp_expires_at = ?, last_login_at = NOW() WHERE id = ?',
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const { rows } = await query(
      'SELECT id, email, first_name, last_name, role, otp, otp_expires_at FROM users WHERE email = ?',
      [email.toLowerCase()]
    );
    ...
    // Clear OTP fields
    await query('UPDATE users SET otp = NULL, otp_expires_at = NULL, is_active = 1 WHERE id = ?', [user.id]);
...
module.exports = { register, login, verifyOtp, refreshToken, logout, getProfile, updateProfile, changePassword };
